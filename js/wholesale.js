...
(function () {
    ...
    const Wholesale = {
        ...
        _showInvoice: function (order) {
            ...
            const receiptExtrasHtml = PharmaFlow.Settings && PharmaFlow.Settings.getReceiptPaymentExtrasHtml
                ? PharmaFlow.Settings.getReceiptPaymentExtrasHtml('rcpt-payment-extra')
                : '';
        ...
        _downloadInvoicePdf: function (order) {
            ...
            const receiptExtrasPdf = PharmaFlow.Settings && PharmaFlow.Settings.getReceiptPaymentExtrasPlainLines
                ? PharmaFlow.Settings.getReceiptPaymentExtrasPlainLines()
                : [];
            if (receiptExtrasPdf.length) {
                doc.setFont('courier', 'bold');
                doc.setFontSize(8);
                receiptExtrasPdf.forEach(function (line) {
                    doc.text(line, 105, fY, { align: 'center' });
                    fY += 4;
                });
                fY += 3;
            }
